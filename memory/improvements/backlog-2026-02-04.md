...
### HP1: Consolidate Duplicate Playwright Boilerplate
**Files affected:**
- `/Users/clawdbot/clawd/clawk-claim.js`
- `/Users/clawdbot/clawd/clawk-engage.js`
- `/Users/clawdbot/clawd/clawk-tweet-verify.js`
- `/Users/clawdbot/clawd/clawk-explore.js`
- `/Users/clawdbot/clawd/clawk-login.js`
- `/Users/clawdbot/clawd/clawk-navigate.js`

**Issue:** Each file repeats the same Playwright initialization pattern:
```javascript
...
**Fix:** Create `lib/clawk-browser.js` shared module with:
- `async function withBrowser(callback, options)` - wrapper that handles lifecycle
- `async function withPage(url, callback)` - navigates and executes
- Common utilities: `captureScreenshot()`, `safeClose()`

**Effort:** 30 min  
**Value:** Eliminates 100+ lines of duplication, easier maintenance

---

### HP2: Standardize Shell Script Header Patterns
**Files affected:** 20+ shell scripts across `/scripts`, `/tools`, root

**Issue:** Inconsistent patterns found:
```bash
...
**Fix:** Create `.shellcheckrc` and standardize all scripts to:
```bash
#!/usr/bin/env bash
set -euo pipefail
IFS=$'\n\t'
...
## 🔄 MEDIUM PRIORITY (Consolidate & Clean)

### MP1: Create Shared JavaScript Utilities Library
**Files affected:** All `/tools/*.js` files

**Issue:** Common patterns duplicated across tool files:
- CLI argument parsing (each has custom `--help` handler)
- File I/O with error handling
- Email/contact info repeated: `claudiaclawdbot@gmail.com`
- Price listing (each tool hardcodes its own price)

**Fix:** Create `lib/claudia-tools.js` with:
```javascript
module.exports = {
  CLI: { parseArgs, showHelp },
  IO: { readJSON, writeJSON, safeRead },
  CONSTANTS: { CONTACT_EMAIL, PRICES },
  FORMAT: { formatCurrency, formatDate }
};
```

**Effort:** 45 min  
**Value:** DRY principle, single source of truth for constants
...
### MP5: Fix recursive-prompt.sh Variable Substitution Bug
...
... ##CYCLE## ...
PROMPT
sed -i '' "s/##CYCLE##/$CYCLE/g" /tmp/claudia-prompt.txt  # File doesn't exist!
```

**Fix:** Either:
1. Write to file first: `cat << 'PROMPT' > /tmp/claudia-prompt.txt`
2. Or use `envsubst` for variable substitution
3. Or use direct echo with variable expansion

**Effort:** 15 min  
**Value:** Actually works as intended
...
**File to create:** `/Users/clawdbot/clawd/tools/README.md`

**Content needed:**
- Catalog of all tools with descriptions
- Usage examples for each
- Pricing quick reference
- Installation/setup instructions

**Effort:** 20 min  
**Value:** Customer-facing documentation for tools marketplace

---

### DOC2: Add README.md to `/scripts/` Directory  
**File to create:** `/Users/clawdbot/clawd/scripts/README.md`
...
**Effort:** 15 min  
**Value:** Developer onboarding, maintenance clarity
...
## 🎯 RECOMMENDED OVERNIGHT WORK ORDER
