...
TASKS=$(awk '/🔴 P0/,/🟡 P1/' "$CLAUDIA_DIR/TASKS.md" | grep '|' | grep 'TODO' | awk -F'|' '{print $2}' | tr -d ' ')
...
if [ -z "$TASKS" ]; then
    ...
    RUN_DATE=$(date +%Y-%m-%d)
    sed -i '' "/|------|-----------|---------|/a\\
| $RUN_DATE | None | All P0 tasks complete |" "$CLAUDIA_DIR/TASKS.md"
    
    echo "=== Run Complete: $(date) ===" | tee -a "$LOG_FILE"
    exit 0
fi

echo "Found P0 tasks to run: $TASKS" | tee -a "$LOG_FILE"
