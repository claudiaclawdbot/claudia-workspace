...
const MEMORY = {
   ...
   * @returns {string} Path to daily memory file
   */
  getDailyFile(date = new Date()) {
    const d = new Date(date);
    const filename = `${d.toISOString().split('T')[0]}.md`;
    return path.join(this.getMemoryDir(), filename);
  },

  /**
   * Write to daily memory file
   * @param {string} content - Content to append
   * @param {Object} metadata - Optional metadata (section, tags, etc.)
   * @returns {string} Path to written file
   */
  writeDaily(content, metadata = {}) {
    const filePath = this.getDailyFile();
    ...
    const existing = IO.safeRead(filePath, `# ${new Date().toDateString()}\n\n`);
   ...
   * @param {string} section - Section name (e.g., "Core Context", "Lessons")
   * @param {string} content - Content to add
   * @param {Object} options - Options
   * @returns {boolean} Success
   */
  writeLongTerm(section, content, options = {}) {
    const filePath = this.getLongTermFile();
    const { append = true, dedupe = true } = options;
    
    if (!fs.existsSync(filePath)) {
      fs.writeFileSync(filePath, `# MEMORY.md — Long-Term Memory\n\n`);
    }
    
    let memory = fs.readFileSync(filePath, 'utf8');
    
    // Check for duplicates if dedupe enabled
    if (dedupe && memory.includes(content.substring(0, 100))) {
      return false;
    }
    ...
    if (memory.includes(sectionHeader)) {
      ...
      if (append) {
        ...
        memory = memory.replace(regex, `$1${entry}`);
      }
    } else {
   ...
   * @returns {Array<{date: string, content: string}>} Recent entries
   */
  getRecent(days = 7) {
    const entries = [];
    ...
    for (const file of files.slice(0, days)) {
      ...
      const content = IO.safeRead(filePath);
   ...
   * @returns {Array<{file: string, matches: Array}>} Search results
   */
  search(query) {
    const results = [];
    const memoryDir = this.getMemoryDir();
...
Usage in your tool:
  const { CLI, IO, FORMAT, LOG, MEMORY, CONSTANTS } = require('../lib/claudia-tools');
...
- MEMORY: writeDaily, writeLongTerm, getRecent, search
